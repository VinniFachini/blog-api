...
<template>
    <div class="bg-slate-700 flex h-screen items-center flex-col justify-center w-screen">
        ...
        <form @submit.prevent="handleLogin(this.loginUsername, this.loginPassword)" method="POST" class="bg-slate-900 p-10 rounded-md">
            ...
            <TextInput class="my-4 text-white" v-model="loginUsername" label="Username" type="text" placeholder="Your Username" :required="true" inputFull/>
            <TextInput class="my-4 text-white" v-model="loginPassword" label="Password" type="password" placeholder="Your Password" :required="true" inputFull/>
